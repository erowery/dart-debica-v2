...
export default function HomePage() {
  ...
  const loadData = useCallback(async (showLoadingIndicator = false) => {
    if (!activeLeague || document.visibilityState === 'hidden') return;
    ...
    try {
      ...
      setApiError('Nie udało się pobrać danych. Sprawdź połączenie.');
      setLoading(false);
    }
  }, [activeLeague]);
  ...
  return (
    <main className="min-h-screen px-4 pb-28 pt-6 max-w-4xl mx-auto">
      ...
      {apiError && (
        <div className="p-4 mb-4 bg-red-500/10 border border-red-500/40 rounded-2xl">
          <p className="text-red-400 text-sm font-semibold mb-2">⚠️ {apiError}</p>
          <button 
            onClick={() => { setApiError(null); loadData(true); }}
