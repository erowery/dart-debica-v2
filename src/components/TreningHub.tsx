...
import { IconStopwatch, IconChevronLeft } from '@/components/icons';
    ...
    <div>
      ...
      <div className="space-y-3">
        <button
          ...
          className="w-full text-left p-5 card-premium transition-all"
        >
          <div className="flex items-center justify-between mb-1">
            <h3 className="text-base font-bold text-white">🎯 Darterska Piramida Treningowa</h3>
            <IconStopwatch className="w-6 h-6 text-brand" />
          </div>
        ...
        <button
          ...
          className="w-full text-left p-5 card-premium transition-all hover:border-red-400/40"
        >
          <div className="flex items-center justify-between mb-1">
            <h3 className="text-base font-bold text-white">🔴 Trening Podwójnych</h3>
            <IconStopwatch className="w-6 h-6 text-red-400" />
          </div>
          <p className="text-sm text-slate-400">
...
function BackButton({ onClick }: { onClick: () => void }) {
  return (
    <button onClick={onClick} className="text-sm text-slate-400 hover:text-slate-200 mb-4 flex items-center gap-1">
      <IconChevronLeft className="w-4 h-4" /> Wróć do wyboru treningu
    </button>
  );
}
