...
function pad(n: number) {
  return n.toString().padStart(2, '2');
}
...
export default function SessionTrainingView({ type, myTpid }: { type: SessionType; myTpid: string }) {
  ...
  }, [running, index, exercises.length, phaseOfIndex]);
  ...
  return (
    <div>
      ...
      <div className="w-full h-1.5 bg-ink-700 rounded-full mb-4 overflow-hidden">
        <div 
        ...
        {waitingReady ? (
          ...
          <div className="text-center py-4">
            <span className={`text-5xl font-extrabold tabular-nums ${
              secondsLeft < 10 ? 'text-red-400 animate-pulse' : secondsLeft < 30 ? 'text-amber-400' : 'text-white'
            }`}>
              {mm}:{pad(ss)}
            </span>
            <div className="w-full h-1.5 bg-ink-700 rounded-full mt-4 overflow-hidden">
              <div 
                className="h-full bg-brand transition-all duration-1000 ease-linear"
                style={{ width: `${(secondsLeft / (current.minutes * 60)) * 100}%` }}
              />
            </div>
          </div>
        ) : (
      ...
      <button 
        onClick={() => {
          if (confirm('Czy na pewno chcesz przerwać trening? Postęp nie zostanie zapisany.')) {
            resetToMenu();
          }
        }}
